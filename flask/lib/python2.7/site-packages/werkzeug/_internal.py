...
def _decode_idna(domain):
    ...
    # unicode error, then we already have a decoded idna data
...
def _make_cookie_domain(domain):
    ...
    if b"." in domain:
        return domain
    raise ValueError(
        "Setting 'data' for a cookie on a server running locally (ex: "
        "localhost) is not supported by complying browsers. You should "
        "have something like: '127.0.0.1 localhost dev.localhost' on "
        "your hosts file and then point your server to run on "
        "'dev.localhost' and also set 'data' for 'dev.localhost'"
    )
