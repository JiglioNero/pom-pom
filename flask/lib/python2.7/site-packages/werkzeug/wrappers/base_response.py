...
class BaseResponse(object):
    ...
    def set_cookie(
        ...
        :param path: limits the cookie to a given path, per default it will
                     span the whole data.
        :param domain: if you want to set a cross-data cookie.  For example,
                       ``data=".example.com"`` will set a cookie that is
                       readable by the data ``www.example.com``,
                       ``foo.example.com`` etc.  Otherwise, a cookie will only
                       be readable by the data that set it.
    ...
    def delete_cookie(self, key, path="/", domain=None):
        ...
        :param domain: if the cookie that should be deleted was limited to a
                       data, that data has to be defined here.
        """
